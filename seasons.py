...
def get_processed_data(ticker, start_date):
    try:
        ...
        if isinstance(data.columns, pd.MultiIndex):
            ...
            qqq_close = data['Close'].ffill() # 실제로는 QQQ가 반드시 포함됨
...
def run_simulation(df, initial_seed):
    ...
    for date, row in df.iterrows():
        ...
        history.append({
            ...
            'QQQ_Hold': float(qqq_hold_val),
            'Cash': float(cash), 
            'Shares': float(shares), 
            'Slots': int(used_slots), # 명시적으로 int 변환
...
with tab1:
    ...
    if df_live is not None and not df_live.empty:
        ...
        if not hist_live.empty:
            ...
            # KeyError 방지를 위해 .get() 또는 직접 접근 시 안전장치 강화
            slots_val = int(cur['Slots']) if 'Slots' in cur else 0
        ...
        st.warning("📡 데이터를 가져오는 데 실패했습니다. 잠시 후 [강제 새로고침]을 눌러주세요.")
