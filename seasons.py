    ...
    if df_live is not None and not df_live.empty:
        ...
        with cl:
            ...
            if slots_val < num_slots:
                ...
                st.caption(f"1회 투입 예산: ${target_slot_cash:,.2f} | 기준 RSI: {rsi_now:.2f}")
...
# --- TAB 2: 백테스트 ---
with tab2:
    st.header(f"🔍 {num_slots}슬롯 RSI 기반 성과 분석")
    c1, c2, c3 = st.columns(3)
    with c1: s_date = st.date_input("테스트 시작일", value=datetime(2013, 1, 1), min_value=datetime(2013, 1, 1), key="final_bt_s")
        ...
        if df_back is not None:
            ...
            if not res_back.empty:
                ...
                for yr in sorted(res_back['year'].unique()):
                    ...
                    temp_seed = y_e_v
                st.subheader("📅 연도별 성과 요약 (오리지널 RSI)")
