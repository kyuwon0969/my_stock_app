...
with st.sidebar:
    ...
    st.divider()
    st.info(f"💵 현재 환율: **{exch_rate:,.2f}원**")
...
with tab1:
    ...
    if res:
        ...
        c4.metric(f"현재가", f"${live:.2f}", f"{(live-p1)*exch_rate:,.0f}원")
...
with tab2:
    ...
    if st.button(f"🚀 {target_ticker} 백테스트 실행"):
        ...
        if df_back is not None:
            ...
            m1.metric("최종 자산", f"${final_val:,.2f}", f"{final_val*exch_rate:,.0f}원")
