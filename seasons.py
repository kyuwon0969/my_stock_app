...
def run_simulation(df, initial_seed, num_slots, pcr=0.7, start_limit_date=None, end_limit_date=None):
    ...
    ivy_reserve = 0.0
    cumulative_withdrawn = 0.0 # [추가] 인출된 총 수익금
...
with st.sidebar:
    ...
    # [PCR 설정 추가]
    pcr_val = st.slider("PCR (재투자 비중)", 0.0, 1.0, 0.7, 0.05, help="수익의 몇 %를 계좌에 남겨 복리로 굴릴지 결정합니다.")
...
with tab1:
    ...
    if raw_df is not None:
        ...
        if not res_live.empty:
            ...
            c4.metric("현재 자산(인출포함)", f"${cur['Total']:,.2f}")
            
            sc1, sc2 = st.columns(2)
            sc1.info(f"🏦 Ivy 비상금(BOXX): **${cur['Ivy']:,.2f}**")
            sc2.success(f"💸 누적 인출 금액(30%): **${cur['Withdrawn']:,.2f}**")
    ...
    if st.button("🚀 백테스트 실행"):
        ...
        if bt_raw is not None:
            ...
            if not res_b.empty:
                ...
                m_r1.metric("최종 창출 가치", f"${final_val:,.0f}")
                m_r2.metric("총 인출 현금", f"${total_withdrawn:,.0f}")
                m_r3.metric("CAGR (연복리)", f"{cagr:.2f}%")
                ...
                m_r8.metric("순수 계좌 잔고", f"${(final_val - total_withdrawn):,.0f}")

                st.line_chart(res_b[['Total', 'QQQ']])
                
                st.markdown("#### 📅 연도별 성과 및 인출 리포트")
                ...
                for yr in sorted(res_b['year'].unique()):
                    ...
                    yearly_stats.append({"연도": yr, "수익률": f"{y_ret:.1f}%", "MDD": f"{y_mdd:.1f}%", "연간 인출액": f"${y_withdrawn:,.0f}"})
