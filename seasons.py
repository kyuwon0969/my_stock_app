...
# 1. 페이지 설정
st.set_page_config(page_title="사계절 전략 매니저 V6.5", page_icon="🌿", layout="wide")
localS = LocalStorage()
...
def run_simulation(df, initial_seed, num_slots, pcr=0.7, start_limit_date=None, end_limit_date=None):
    ...
    ivy_reserve = 0.0
    safe_cash = 0.0 # PCR 30%로 모인 안전 자산
    ...
    for date_idx, row in sim_df.iterrows():
        ...
        if shares > 0 and curr_c >= s_l:
            ...
            if profit > 0:
                ...
                safe_cash += with_profit # 30% 적립
            else:
                cash += profit # 손실 시 즉시 현금 차감 (보험금 수령 기능 제거)
            
            shares, used_slots, slot_cash, avg_price, slot_details = 0.0, 0, 0.0, 0.0, []
            sold = True
        
        # [핵심] 2. Tulip 진입 시 Ivy 비상금 + Safe Cash 일괄 투입
        if used_slots == 0 and mode == "Tulip":
            if ivy_reserve > 0 or safe_cash > 0:
                cash += (ivy_reserve + safe_cash)
                ivy_reserve = 0.0
                safe_cash = 0.0
        ...
        history.append({
            ...
            'Slots': used_slots, 'Avg': avg_price, 'Safe': safe_cash,
...
with st.sidebar:
    ...
    config_key = f"v6_5_tulip_{target_ticker}"
...
with tab1:
    ...
    if raw_df is not None:
        ...
        if not res_live.empty:
            ...
            sc2.success(f"🛡️ 안전 현금(PCR 30%): **${cur['Safe']:,.2f}**")
            ...
            with g1:
                ...
                if cur['Slots'] < num_slots:
                    ...
                    st.write(f"**타점:** `${b_p:.2f}` 이하 | **수량:** `{int(t_cash // b_p)} 주`")
                else: st.write("✅ 매수 완료")
            with g2:
...
with tab2:
    ...
    bt_start = col_b1.date_input("시작일", value=date(2013, 1, 1))
    bt_end = col_b2.date_input("종료일", value=date.today())
    ...
    if st.button("🚀 백테스트 실행"):
        ...
        if bt_raw is not None:
            ...
            if not res_b.empty:
                ...
                m1.metric("최종 자산", f"${final_val:,.0f}"); m2.metric("총 수익률", f"{total_ret:.1f}%")
                m3.metric("CAGR", f"{cagr:.2f}%"); m4.metric("승률", f"{win_rate:.1f}%")
                
                m5, m6, m7, m8 = st.columns(4)
                m5.metric("MDD", f"{mdd:.2f}%"); m6.metric("Average DD", f"{avg_dd:.2f}%")
                m7.metric("손익비", f"{pl_ratio:.2f}"); m8.metric("안전 현금", f"${res_b['Safe'].iloc[-1]:,.0f}")
                ...
                for yr in sorted(res_b['year'].unique()):
                    ...
                    y_stats.append({"연도": yr, "수익률": f"{y_ret:.1f}%", "MDD": f"{y_mdd:.1f}%", "대기 자금": f"${(y_df['Ivy'].iloc[-1] + y_df['Safe'].iloc[-1]):,.0f}"})
