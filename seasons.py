...
# 1. 페이지 설정
st.set_page_config(page_title="사계절 전략 Pro V7", page_icon="🌿", layout="wide")
localS = LocalStorage()
...
def get_processed_data(ticker, start_date):
    try:
        ...
        # 대상 종목, QQQ(판단용), QLD(수익금 투자용) 수집
        data = yf.download([ticker, "QQQ", "QLD"], start=fetch_start, progress=False)
        if data.empty: return None
        
        if isinstance(data.columns, pd.MultiIndex):
            target_close = data['Close'][ticker].ffill()
            qqq_close = data['Close']['QQQ'].ffill()
            qld_close = data['Close']['QLD'].ffill()
        else:
            target_close, qqq_close, qld_close = data['Close'].ffill(), data['Close'].ffill(), data['Close'].ffill()
            
        df = pd.DataFrame(index=target_close.index)
        df['close'], df['qqq_close'], df['qld_close'] = target_close, qqq_close, qld_close
...
def run_simulation(df, initial_seed, num_slots, pcr=0.7, start_limit_date=None, end_limit_date=None):
    ...
    ivy_reserve = 0.0
    qld_shares = 0.0 # [추가] QLD 보유 수량
    pending_qld_purchase = 0.0 # 익절 다음날 살 금액
    ...
    for date_idx, row in sim_df.iterrows():
        ...
        # 2. [추가] QLD 매수 집행 (익절 다음 날 전날 확정된 금액으로 매수)
        if pending_qld_purchase > 0:
            qld_buy_price = row['qld_close']
            qld_shares += pending_qld_purchase / qld_buy_price
            pending_qld_purchase = 0.0
        ...
        if shares > 0 and curr_c >= s_l:
            ...
            if profit > 0:
                ...
                # [수정] 30% 수익금은 내일 QLD 매수 대기
                pending_qld_purchase = with_profit
            else:
                cash += profit # 손실 반영
        ...
        # 4. Tulip 진입 시 Ivy 비상금만 투입 (QLD는 바이앤홀드이므로 건드리지 않음)
        ...
        qld_val = qld_shares * row['qld_close']
        total_assets = cash + (shares * curr_c) + ivy_reserve + qld_val
        history.append({
            'Date': date_idx, 'Total': total_assets, 'Cash': cash, 'Shares': shares, 
            'Slots': used_slots, 'Avg': avg_price, 'QLD_Val': qld_val,
...
with st.sidebar:
    ...
    config_key = f"v7_qld_{target_ticker}"
...
with tab1:
    ...
    if raw_df is not None:
        ...
        if not res_live.empty:
            ...
            st.subheader(f"📊 {target_ticker} + QLD 하이브리드 현황")
            ...
            sc2.success(f"🚀 QLD 누적 보유액: **${cur['QLD_Val']:,.2f}**")
...
with tab2:
    st.header("🔍 과거 데이터 기반 백테스트 (QLD 엔진)")
    ...
    if st.button("🚀 백테스트 실행"):
        ...
        if bt_raw is not None:
            ...
            if not res_b.empty:
                ...
                st.subheader("🏆 하이브리드 전략 백테스트 결과")
                m1, m2, m3, m4 = st.columns(4)
                m1.metric("최종 자산", f"${final_val:,.0f}"); m2.metric("CAGR", f"{cagr:.2f}%")
                m3.metric("MDD", f"{mdd:.2f}%"); m4.metric("승률", f"{win_rate:.1f}%")
                ...
                for yr in sorted(res_b['year'].unique()):
                    ...
                    y_stats.append({"연도": yr, "수익률": f"{y_ret:.1f}%", "MDD": f"{y_mdd:.1f}%", "QLD 비중": f"${y_df['QLD_Val'].iloc[-1]:,.0f}"})
