...
def get_processed_data(ticker, start_date):
    try:
        ...
        # [데이터 시점 로직] p1은 어제, p2는 그저께
        df['p1_raw'] = df['close'].shift(1)
        df['p2_raw'] = df['close'].shift(2)
        ...
        df['rsi_live'] = (100 - (100 / (1 + (gain / loss.replace(0, np.nan))))).shift(1)
...
# --- 시뮬레이션 엔진 ---
def run_simulation(df, initial_seed, num_slots, pcr=0.7, start_limit_date=None, end_limit_date=None, pending_dep=0.0):
    ...
    for date_idx, row in sim_df.iterrows():
        ...
        p1, p2, curr_c, rsi_v = row['p1_raw'], row['p2_raw'], row['close'], row['rsi_sim']
        ...
        if shares > 0 and curr_c >= s_l:
            ...
            if profit > 0:
                comp_p, with_p = profit * pcr, profit * (1 - pcr)
                if mode == "Ivy": ivy_reserve += comp_p
                else: cash += comp_p
                cumulative_withdrawn += with_p
            else: cash += profit
        ...
        history.append({'Date': date_idx, 'Total': total_assets, 'Cash': cash, 'Shares': shares, 'Slots': used_slots, 'Avg': avg_price, 'Withdrawn': cumulative_withdrawn, 'QQQ': (initial_seed / qqq_start_p) * row['qqq_close'], 'Ivy': ivy_reserve})
...
with st.sidebar:
    ...
    p_dep = st.number_input("추가 입금/인출액 ($)", value=float(get_setting('pending_dep', 0.0)), help="전량 매도 후 반영됩니다. 인출은 음수 입력.")
...
with tab1:
    ...
    if raw_df is not None:
        ...
        if not res_live.empty:
            ...
            p1_val, p1_date = latest_row['p1_raw'], raw_df.index[-1].strftime('%Y-%m-%d')
            p2_val, p2_date = latest_row['p2_raw'], raw_df.index[-2].strftime('%Y-%m-%d')
